#!/usr/bin/env python3
"""
Appediet Server & API Proxy
Serves the PWA web application and provides AI nutrition analysis endpoints.
Supports local network IP and Cloudflare HTTPS tunnel for Android installation.
"""
...
def load_dotenv_if_exists():
    """Load key-values from .env file into os.environ if present"""
...
class AppedietHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        # API: Analyze Food with Gemini Vision
        if self.path == '/api/analyze-food':
            self.handle_analyze_food(req_data)
            return

        # API: Coach Chat with Gemini
        if self.path == '/api/coach-chat':
  ...
  "insights": {{
    ...
    "what_to_eat_next": "شرب كوب ماء كبير مع سناك فاكهة أو زبادي بعد ساعتين."
        ...
        try:
            ...
            # Fallback to balanced meal preset if offline or parsing error
            resp_json = {
...
def run_server():
    ...
    try:
        with socketserver.TCPServer(("0.0.0.0", PORT), handler) as httpd:
            print("=" * 60)
            print("  [OK] Raheem Coach Server is running!")
            print("=" * 60)
            print(f"  Mobile Wi-Fi URL: http://{local_ip}:{PORT}")
            print(f"  Desktop URL:     http://localhost:{PORT}")
            print("=" * 60)
            print("  To install on Android:")
            print("  1. Open the URL in Chrome or Mi Browser on your phone.")
            print("  2. Tap 'Install App' or 'Add to Home Screen'.")
            print("=" * 60)
            httpd.serve_forever()
    except Exception as e:
